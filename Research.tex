...
%The error in the first-order approximation is $\sum_{i=2}^\infty (\inv M E)^i \inv M = (\inv M E)^2\sum_{i=0}^\infty (\inv M E)^i \inv M = \inv M E \inv M E \inv{(M-E)}$. We can bound the relative error of this approximation as follows:
